...
turtles-own[adopted? ];; add a property related to adoption, ? means boolean

;; tell Netlogo that we want 2 types of agents
breed[influentials influential] ;; specify the plurals and singlulars
...
to setup
  ...
  ;; set the default shape of breeds respectively
  set-default-shape regulars "person"
  set-default-shape influentials "star"

  ;; create the network based on the chooser
  if network = "random" [
  ...
  ;; create the netwrok based on a preferential attachment network
  ...
  ;;define who are influentials, who are regulars
  ask n-of (fraction-of-influentials * num-agents) turtles [
...
to go
  ...
  ask turtles with [not adopted?][ ;; make sure only not adopted ones can have the action of adopt
    adopt ;; this adopt procedure is defined below in ' to adopt'

  ]
  tick ;; count how many ticks
...
to adopt ;; define a new procedure to determine whether or not to adopt ;; ask the turtle to adopt or not adopt randomly

  ;; adopt based on the broadcast influence

  if random-float 1.0 < broadcast-influence [
    set adopted? true
    set color red ;; add visual element to distinguish adopted
  ]



  ;; adopt based on the social influence based on a network and exclude ones that have adopted the broadcast influence
  ...
  if (breed = regulars) [
    ...
    ;; two types of neighbors, adopting from regulars, and ones adopting from influentials
    ;; count the social influence of influentials and regular neighbors respecitvely.
...
<experiments>
  <experiment name="social-influence-experiment" repetitions="10" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="fraction-of-influentials">
      <value value="0.1"/>
    </enumeratedValueSet>
